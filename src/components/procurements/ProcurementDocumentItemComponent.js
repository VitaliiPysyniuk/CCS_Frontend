...
export const ProcurementDocumentItemComponent = ({item, number, onTMVInput, onWarehouseInput, onItemDelete,
                                                     onCostChange, onNumberChange, onDataChange, onCommentChange }) => {


    const onDateFilesChange = (event) => {
        onDataChange(event, item)
    ...
    return (
        <tr>
            ...
            <td className="p-0 ps-1 align-middle">
                <input id="cost" className="w-100 p-0 bc_none" defaultValue={item.cost_per_unit}
                       onChange={(e) => onCostChange(e, item)}/>
            </td>
            <td className="p-0 ps-1 align-middle">
                <input id="warehouse" className="w-100 p-0 bc_none" defaultValue={item.warehouse.name}
                       onSelect={() => onWarehouseInput(item)}/>
            </td>
            <td className="p-0 ps-1 align-middle">
